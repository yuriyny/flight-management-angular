import { Component, OnInit } from '@angular/core';
import { Router } from '@angular/router';
import { FlightInfoDto } from '../flight-list/flight-info-dto';
import { FlightService } from '../flight-list/flight.service';
import { TwoWayFlightInfoDto } from '../flight-list/two-way-flight-info-dto';
import { AddReservationDto } from '../reservation-list/add-reservation-dto';

@Component({
  selector: 'app-flight-search-return-result',
  templateUrl: './flight-search-return-result.component.html',
  styleUrls: ['./flight-search-return-result.component.scss']
})
export class FlightSearchReturnResultComponent implements OnInit {

  selectedClass!: number;
  selectedFlight!: AddReservationDto;
  flights!: TwoWayFlightInfoDto;
  constructor(private flightService: FlightService, private router: Router) { }

  ngOnInit(): void {
    if(this.flightService.isRefreshed){
      this.router.navigateByUrl('/home/search-flight');
    }
    this.flights = this.flightService.roundFlightSearchResult;
    console.log(this.flights);
  }

  bookFlight(flight: FlightInfoDto){
    if(this.selectedClass == 0){
      this.flightService.selectedFlights.returnTicketId = flight.economyTicketPriceId; 
    }
    if(this.selectedClass == 1){
      this.flightService.selectedFlights.returnTicketId = flight.businessTicketPriceId;
    } 
    this.flightService.addReservation(this.flightService.selectedFlights).subscribe(data => {
      console.log(this.flightService.selectedFlights);
      
      console.log("Saved");
      //this.router.navigateByUrl('/home/my-reservations');   
    }); 
    console.log("============");
    
      console.log(this.flightService.selectedFlights);
      console.log("------");
      
       
  }

}
